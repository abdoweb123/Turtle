...
@section('content')
    ...
    @endif
<form method="POST" action="{{ route('admin.settings.update',['id' => 1, 'type' => $type]) }}" enctype="multipart/form-data" >
    @csrf
    @method('PUT')
    <div class="row">
        @foreach ($Models as $Setting)
            ...
            @elseif (str_contains($Setting['key'], '_services') || (str_contains($Setting['key'], 'ar') || str_contains($Setting['key'], 'en') || $Setting['type'] == 'meta') && !str_contains($Setting['key'], 'title_'))
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang('trans.'.$Setting['key'])</label>
                        <textarea style="min-height: 300px" class="editor" id="{{ $Setting['key'] }}" name="{{ $Setting['key'] }}" required placeholder="@lang('trans.' . $Setting['key'])" rows="10" cols="80">{!! $Setting['value'] !!}</textarea>
                    </div>
                </div>
